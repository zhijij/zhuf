...
def search_milvus(query: str, role: str | None = None, limit: int = 6) -> list[dict[str, Any]]:
...
def rag_prefetch(query: str, role: str | None = None) -> dict[str, Any]:
    hits = search_milvus(query, role)
    source = "milvus"
    if not hits:
        hits = [normalize_hit(item) for item in search_vector_knowledge(query, role)]
        source = "pgvector"
    return {
        "source": source,
        "hits": hits[:6],
