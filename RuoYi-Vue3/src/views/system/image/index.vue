<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="imageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型:0普通,1封面,2户型图" align="center" prop="imageType" />
      <el-table-column label="排序" align="center" prop="sortNo" />
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:image:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:image:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除房源图片编号为"' + _imageIds + '"的数据项？').then(function() {
...
function handleExport() {
  proxy.download('system/image/export', {
    ...queryParams.value
